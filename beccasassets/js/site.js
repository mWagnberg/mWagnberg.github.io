...
function artMarkup(product) {
    // Stödj product.images array för carousel
    if (product.images && Array.isArray(product.images) && product.images.length > 0) {
        ...
        const navButtons = product.images.length > 1 ? `
        ...
        return `
            <div class="product-art product-art-carousel" id="${carouselId}" data-total-images="${product.images.length}">
    ...
    if (product.image) {
        ...
        <div class="product-art product-art-image-frame">
            <img class="product-image" src="${normalizeImagePath(product.image)}" alt="${escapeHtml(product.name)}">
        </div>
    `;
    }
